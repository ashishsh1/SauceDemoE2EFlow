...
class CheckoutPage extends BasePage{
    ...
    async getOrderSummary() {
        ...
        // Read and normalize the subtotal,price,tac etc
        const subtotal = this.normalizePrice(await this.subtotal.textContent());

        const tax = this.normalizePrice(await this.tax.textContent());

        const total = this.normalizePrice(await this.total.textContent());

        // Return the complete order information.
        return {products,subtotal,tax,total};
    }
    ...
    normalizePrice(text) {
        ...
        // Convert the extracted amount to a number and format it to exactly two decimal places.
        return `$${Number(match[0].replace('$', '')).toFixed(2)}`;
    }
